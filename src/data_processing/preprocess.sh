...
process_file() {
    ...
    texts=$(echo "$content" | python3 -c "import emoji; print(emoji.replace_emoji(input(), replace=lambda chars, data_dict: f'\t{chars[0]}\n').strip())")
...
run_script() {
	...
	find "$folder_path" -type f -name '*' | parallel process_file
